...
volatile uint8_t stepper_do_step = 0;

volatile uint8_t fullstep_doublephase [4] = {0b1001, 0b1010, 0b0110, 0b0101};
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  	...
  	stepper_actor_init(&regeneration1, 200, 200, 0);


  	stepper_actor_set_position(&regeneration1, 1, 2000);
  	stepper_actor_set_position(&regeneration1, 2, 4000);
  	stepper_actor_set_position(&regeneration1, 3, 6000);
  	stepper_actor_set_position(&regeneration1, 4, 8000);
  	stepper_actor_set_position(&regeneration1, 5, 10000);

  	stepper_actor_set_target(&regeneration1, 4);
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		if (stepper_do_step) {
		    ...
		    if (regeneration1.steppcounter > regeneration1.targetposition) {
		        stepper_actor_get_decrease_step(&regeneration1);
		    } else if (regeneration1.steppcounter < regeneration1.targetposition) {
		        stepper_actor_get_increase_step(&regeneration1);
		    } else {
		        regeneration1.step = 0;
		        stepper_actor_set_step(&regeneration1, 0); // Motor aus
		    }
		}
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, Stepper_B4_uC_Pin|Stepper_A1_uC_Pin|Stepper_A3_uC_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOE, debug_fault_Pin|debug_heartbeat_Pin|debug_1_Pin|debug_2_Pin
                          |debug_3_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(Stepper_B2_uC_GPIO_Port, Stepper_B2_uC_Pin, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin = Stepper_B4_uC_Pin|Stepper_A1_uC_Pin|Stepper_A3_uC_Pin;
  ...
  /*Configure GPIO pins : debug_fault_Pin debug_heartbeat_Pin debug_1_Pin debug_2_Pin
                           debug_3_Pin */
  GPIO_InitStruct.Pin = debug_fault_Pin|debug_heartbeat_Pin|debug_1_Pin|debug_2_Pin
                          |debug_3_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
